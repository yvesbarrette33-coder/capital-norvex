#!/usr/bin/env python3
"""Seed 3 family offices Tier 2 PM 2026-05-12 + queue drafts.

Vague #2 du plan séquentiel — family offices "solides" identifiés via WebSearch+Hunter.
- Westmont Hospitality (Fereed Mangalji, famille) — ON / EN
- Letko Brosseau (Jean Gattuso, Chairman) — QC / FR
- Foster & Associates (Karen Parente, VP Ops) — ON / EN
"""
...
TARGETS: List[Dict[str, Any]] = [
    {
        "name": "Fereed Mangalji",
        "organization": "Westmont Hospitality Group",
        "title": "Managing Director (Mangalji family)",
        "region": "ON",
        "language": "en",
        "email": "fereed.mangalji@whg.com",
        "tier": "2_family_office_PM_2026-05-12",
    },
    {
        "name": "Jean Gattuso",
        "organization": "Letko, Brosseau & Associés",
        "title": "Président du conseil",
        "region": "QC",
        "language": "fr",
        "email": "jean.gattuso@lba.ca",
        "tier": "2_family_office_PM_2026-05-12",
    },
    {
        "name": "Karen Parente",
        "organization": "Foster & Associates (Foster Family Office)",
        "title": "Vice President of Operations",
        "region": "ON",
        "language": "en",
        "email": "kparente@fostergroup.ca",
        "tier": "2_family_office_PM_2026-05-12",
    },
]
...
def _find_existing(name: str, organization: str) -> Optional[str]:
    """Cherche un doc existant par nom (case-insensitive)."""
    name_l = (name or "").lower().strip()
    # Query all capitalTargets and filter in memory (small enough)
...
def main():
    ...
    print(f"=== Seed family offices T2 PM 2026-05-12 — {len(TARGETS)} cibles ===\n")
    results = []
