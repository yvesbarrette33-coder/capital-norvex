"""Triage Béatrice — Sonnet 4.6 sur emails entrants yves@."""
...
def triage_email(
    *,
    ...
    """Trie un email et retourne dict structuré (calque Sophie/Camille)."""
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        raise RuntimeError("ANTHROPIC_API_KEY manquant — Béatrice ne peut pas trier")
    ...
    if start == -1 or end == -1:
        raise RuntimeError(f"Triage Béatrice : JSON manquant. Reçu : {text[:200]!r}")

    parsed = json.loads(text[start: end + 1])
    parsed.setdefault("category", "autre_general")
    parsed.setdefault("priority", "normale")
    parsed.setdefault("language", "fr")
    parsed.setdefault("isPersonal", False)
    parsed.setdefault("summary", "")
    parsed.setdefault("actionRequested", "")
    parsed.setdefault("deadlineMentioned", None)
    # Béatrice : autoSendSafe TOUJOURS False (autonomie moyenne, Yves approuve)
    parsed["autoSendSafe"] = False
