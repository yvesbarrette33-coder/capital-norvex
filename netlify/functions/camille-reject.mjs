/**
 * GET  /api/camille-reject?draft=<id>&exp=<iso>&token=<hmac>
 * POST /api/camille-reject  body: { draftId, reason? } + header X-Internal-Secret
 *
 * Rejette un draft Camille (sans envoyer). Audit trail complet.
 */
  ...
  try {
    ...
    return htmlResponse(
      ...
      `<p class="err">${result.message}</p>
       <a class="btn" href="/camille-admin.html">Dashboard</a>`,
      result.status
    );
  } catch (e) {
    return isPost
