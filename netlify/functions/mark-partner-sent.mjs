/**
 * POST /.netlify/functions/mark-partner-sent
 * Header: X-Internal-Secret
 * Body: { dossierId }
 *
 * Sets partnerSummarySent=true on a Firestore dossier.
 * Called by the Python agent after successfully sending the executive summary.
 */
...
export default async (req) => {
  ...
  const docUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/dossiers/${dossierId}?updateMask.fieldPaths=partnerSummarySent&updateMask.fieldPaths=partnerSentAt&currentDocument.exists=true`;
  ...
  try {
    const resp = await fetch(docUrl, {
      ...
      body: JSON.stringify({
        fields: {
          partnerSummarySent: { booleanValue: true },
          partnerSentAt:      { timestampValue: new Date().toISOString() },
        },
      }),
    });
    if (!resp.ok) {
