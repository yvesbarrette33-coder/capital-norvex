/**
 * GET  /api/sophie-reject?draft=<id>&exp=<iso>&token=<hmac>
 * POST /api/sophie-reject  body: { draftId, reason? } + header X-Internal-Secret
 *
 * Rejette un draft Sophie (sans envoyer). Audit trail complet.
 */
  ...
  try {
    ...
    return htmlResponse(
      ...
      `<p class="err">${result.message}</p>
       <a class="btn" href="/sophie-admin.html">Dashboard</a>`,
      result.status
    );
  } catch (e) {
    return isPost
