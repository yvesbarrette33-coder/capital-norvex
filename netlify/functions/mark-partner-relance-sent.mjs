/**
 * POST /.netlify/functions/mark-partner-relance-sent
 * Header: X-Internal-Secret
 * Body: { dossierId, level }
 *
 * Met a jour le dossier Firestore:
 *   - partnerRelanceLevel = level
 *   - lastPartnerRelanceAt = serverTimestamp (ISO now)
 */
...
export default async (req) => {
  ...
  if (!lvl || lvl < 1 || lvl > 3) return json({ error: "Invalid level (1-3)" }, 400);
  ...
  const docUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/dossiers/${dossierId}?updateMask.fieldPaths=partnerRelanceLevel&updateMask.fieldPaths=lastPartnerRelanceAt&currentDocument.exists=true`;
  ...
  try {
    const resp = await fetch(docUrl, {
      ...
      body: JSON.stringify({
        fields: {
          partnerRelanceLevel:  { integerValue: String(lvl) },
          lastPartnerRelanceAt: { timestampValue: new Date().toISOString() },
        },
      }),
    });
    if (!resp.ok) {
