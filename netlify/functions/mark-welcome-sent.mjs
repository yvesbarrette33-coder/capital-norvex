/**
 * POST /.netlify/functions/mark-welcome-sent
 * Header: X-Internal-Secret
 * Body: { dossierId }
 *
 * Sets welcomeEmailSent=true and stage='docs' on a Firestore dossier.
 */
...
export default async (req) => {
  ...
  const docUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/dossiers/${dossierId}?updateMask.fieldPaths=welcomeEmailSent&updateMask.fieldPaths=welcomeEmailSentAt&updateMask.fieldPaths=stage&currentDocument.exists=true`;
    ...
    const resp = await fetch(docUrl, {
      ...
      body: JSON.stringify({
        fields: {
          ...
          welcomeEmailSentAt:  { timestampValue: new Date().toISOString() },
          stage: { stringValue: "docs" },
        },
      }),
    });
    if (!resp.ok) {
