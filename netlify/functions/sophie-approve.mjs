/**
 * GET  /api/sophie-approve?draft=<id>&exp=<iso>&token=<hmac>
 * POST /api/sophie-approve  body: { draftId } + header X-Internal-Secret
 *
 * Approuve un draft Sophie et l'envoie immédiatement via Microsoft Graph
 ...
 * Mode 2 (dashboard)  : POST authentifié depuis le dashboard /sophie-admin.
 */
...
async function approveAndSend(projectId, fsToken, draftId) {
  ...
  // 🐛 Bug fix 2026-05-05 PM : route via SendGrid pour externes (bypass HRDP
  // 5.7.708), Graph pour internes. Identique à Camille/Béatrice.
    ...
    if (!v.ok) {
      return htmlResponse(
        ...
        `<p class="err">⛔ Ce lien d'approbation est invalide ou expiré.</p>
         ...
         <a class="btn" href="/sophie-admin.html">Ouvrir le dashboard Sophie</a>`,
        401
      );
    }
  }
    ...
    return isPost
      ...
      : htmlResponse(
          ...
          `<p class="err">Erreur : ${e.message}</p>
           <a class="btn" href="/sophie-admin.html">Dashboard Sophie</a>`,
          500
        );
  }
}
