/**
 * GET /api/track-job-status?jobId=tjob_xxx
 * Header: x-internal-secret
 *
 * Retourne le statut d'un job Norvex Track background.
 * Lecture seule de Firestore collection `trackJobs/{jobId}`.
 *
 * Status possibles :
 *   - "pending"  : job créé, background pas encore démarré
 *   - "running"  : background actif
 *   - "done"     : terminé avec succès, voir `result` (verdict, stats, ecarts, alertes...)
 *   - "error"    : terminé avec erreur, voir `error`
 *
 * Created 2026-05-19 — Fix timeout 504 Norvex Track via pattern background.
 */
...
export default async (req) => {
  ...
  try {
    ...
    const fsUrl = `https://firestore.googleapis.com/v1/projects/${sa.project_id}/databases/(default)/documents/trackJobs/${encodeURIComponent(jobId)}`;
